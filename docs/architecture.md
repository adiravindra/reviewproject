...
The dashboard first loads choices from `GET /api/import/options`, which does not check credentials or contact a provider. The form uses one generic **Source URL** field. One explicit `POST /api/import` selects a replaceable adapter through the registry: Amazon uses `ApifyAmazonReviewsAdapter`, while Google Maps uses `ApifyGoogleMapsAdapter`. Both expose the shared 10/20/50/100 limits from the provider-neutral contract.
...
`ApifyAmazonReviewsAdapter` calls `automation-lab/amazon-reviews-scraper` with one ASIN, marketplace `US`, the selected `maxReviewsPerProduct`, and `sort: "helpful"`. Its provider key is `apify_automation_lab_amazon`. No star or keyword filter is sent.

`ApifyGoogleMapsAdapter` calls `compass/google-maps-reviews-scraper`, keeps `reviewsSort: "mostRelevant"`, and sends no rating filter. Both adapters preserve provider order through the shared normalizer. Both adapters read only backend `APIFY_API_TOKEN`; Google input also sets `personalData: false` and Google-only review origin. Automation Lab may return public reviewer fields transiently, but the adapters decode only product title, review title, review text, rating, and date. Both adapters discard reviewer identities, profiles, avatars, media, variations, helpful-vote data, owner responses, provider IDs, and raw responses. They require no Amazon or Google account credentials, cookies, browser state, or session tokens.
